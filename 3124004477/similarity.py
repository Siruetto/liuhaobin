...
1. 重复率采用"最长公共子序列"的 Dice 系数:
   ...
   本模块的重复率一律是百分数: 0 表示两篇毫无关系, 100 表示完全一致,
   例如 89.12 表示重复率 89.12%。它不是 0 ~ 1 之间的比例值,
   写进答案文件的也是这个百分数。
...
def duplication_rate(original_text, copied_text):
    ...
    返回值是百分数, 取值范围 0 ~ 100, 例如 89.12 表示 89.12%;
    它不是 0 ~ 1 的比例值。输入是两篇文档的原始文本, 函数内部先做
    归一化; 两篇都为空时定义为完全一致, 返回 100.0。
    """
    ...
    if total == 0:
        return 100.0
    # 2 * LCS / total 是 0 ~ 1 的相似度, 再乘 100 换成百分数,
    # 所以系数写作 200.0 而不是 2.0
    return 200.0 * lcs_length(original, copied) / total


def format_rate(rate):
    """把百分数形式的重复率格式化成保留两位小数的字符串, 如 89.12。"""
